...
def generate_report(report_id):
    # Get the latest timestamp as "now"
    now_utc = StoreStatus.objects.aggregate(Max('timestamp_utc'))['timestamp_utc__max']
    if not now_utc:
        with open(os.path.join(settings.BASE_DIR, 'reports', f'{report_id}.txt'), 'w') as f:
            f.write('No data')
        return
    ...
    store_ids = StoreStatus.objects.values('store_id').distinct().values_list('store_id', flat=True)
    ...
    with open(path, 'w', newline='') as f:
        ...
        for store_id in store_ids:
            # Get timezone, default to America/Chicago
            ...
            # Get business hours, default to 24/7
            ...
            writer.writerow([store_id, round(u_h_min), round(u_d_min / 60), round(u_w_min / 60),
